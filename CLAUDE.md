...
app/MeetingTranscriber/    # Swift macOS menu bar app (SPM)
  ...
  Sources/
    ...
    AppSettings+RPC.swift  # RPC settings snapshot helper for DebugRPCServer (#if !APPSTORE)
    ...
    Settings/
      ...
      HelpBadge.swift            # Clickable ⓘ badge that shows a settings option explanation in a popover (issue #505)
      SettingsHelp.swift         # Plain-English help text strings for settings options (surfaced via HelpBadge)
    ...
    StageTimingStats.swift  # Pipeline stage timing tracking + aggregation (stage_timing.jsonl, feeds ProcessingStatsView)
...
scripts/
  ...
  e2e-permission-health.sh # E2E test for permission-health probes (screen recording + mic report healthy via RPC /state)
  ...
  build_perf_report.py     # CI timing trend analysis helper: reads per-job timing JSON-lines, emits Markdown trend report
